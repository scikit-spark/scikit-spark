...
class ResourceWarningTests(PySparkTest):
    ...
    def test_return_train_score_warn(self):
        ...
        estimators = [
            GridSearchCV(LinearSVC(random_state=0), grid, cv=3, spark=True, iid=True),
            RandomizedSearchCV(
                LinearSVC(random_state=0), grid, n_iter=2, cv=3, spark=True, iid=True)
        ]
        ...
        for estimator in estimators:
            for val in [True, False, 'warn']:
                ...
                result[val] = assert_no_warnings(
                    estimator.fit, X, y,
                    skip_warning_classes=(ConvergenceWarning, ResourceWarning))\
                    .cv_results_
